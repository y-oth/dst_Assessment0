...
## Home and Toss advantage

During the literature review I found a paper by Kaluarachchi and Aparna [1] trying to investigate the effects of home advantage and winning the toss on the outcome of the game. I attempt to investigate that relation using this dataset.
...
data %>%
  ...
  summarise(avg_runs = mean(runs_off_bat + extras, na.rm = TRUE)) %>%
  ggplot(aes(x = over, y = avg_runs, color = factor(innings))) +
  geom_line(size = 1) +
  labs(title = "Average Runs per Over by Innings",
       x = "Over", y = "Average Runs", color = "Innings") +
  theme_minimal()
