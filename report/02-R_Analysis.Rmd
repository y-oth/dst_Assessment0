...
demonstrate that structure and investigate any interesting features. Bukiet [5] has an interesting approach to trying to model this time dependent structure of cricket by using homogeneous Markov Chains. It suggests a Markov Chain were transition probabilities are the runs scored at ball t, and differs depending on who the batter is.
...
to be more conservative as they know what goal they need to reach. This also demonstrates that the homogeneous Markov Chain model as suggested by Bukiet is inappropriate. They do, however, acknowledge this and suggest the inclusion of a score dependence element to make it a time-inhomogeneous Markov chain instead (ie transition probabiliies change with time but are still independent of the past).
