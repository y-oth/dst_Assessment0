...
data %>%
  ...
  group_by(toss_decision, over) %>%
  summarise(avg_runs = mean(runs_off_bat + extras, na.rm = TRUE)) %>%
  ggplot(aes(x = over, y = avg_runs, color = toss_decision)) +
  geom_line(size = 1) +
  labs(title = "Average Runs per Over by Toss Decision",
       x = "Over", y = "Average Runs", color = "Toss Decision") +
  theme_minimal()
